...
def conv_layer(inputs, filters, kernel_size, strides, gain=1.0):
    return tf.keras.layers.Conv2d(inputs=inputs,
                                  filters=filters,
                                  kernel_size=kernel_size,
                                  strides=(strides, strides),
                                  activation=tf.nn.relu,
                                  kernel_initializer=tf.keras.orthogonal_initializer(gain=gain))


# Fully connected layer
def fc_layer(inputs, units, activation_fn=tf.nn.relu, gain=1.0):
    return tf.keras.layers.Dense(inputs=inputs,
                                 units=units,
                                 activation=activation_fn,
                                 kernel_initializer=tf.keras.orthogonal_initializer(gain))
